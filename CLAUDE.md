...
## Receipts films (`data/films/`, `public/films/`)
An article can open with a **Receipts** film: a 40-60 s painted, narrated animation of the article's own argument in its own numbers — a full-bleed word punch as the hook, two to four beats (claim, stat, versus, tally, list), a verdict, and a till receipt of every figure shown. Every frame is painted from scratch in Canvas2D (watercolour, boiling ink, paper), Clawd presents each beat, a Kokoro voice narrates it, and a synthesized score and effects land on the picture's own cues. These are separate from the hand-drawn article films above: those are bespoke explanations; these are one engine and one storyboard per article.

- **You write a storyboard, not code.** `data/films/<slug>.json`; the contract — scene types, word limits, truth rules, palettes, narration — is `brand-crew/skills/receipt-films/SKILL.md`. The engine owns the look and the sound. **Storyboards exist for every article; films are rendered for a chosen set** (recent and featured), because each is ~1.3 MB. An unrendered storyboard is still checked and renders with one command.
- **The film is held to the article.** `pnpm validate:films` fails if any digit on screen or in the narration is absent from the article's MDX, if a key figure (stat value, versus value, tally count) does not sit within ~160 characters of a word from its own label, if a `claim.quote` is not a verbatim run of the article's words, or if a stamp label is outside Measured / Reported / Reasoned / holds / does not hold / half true. For a rendered film it also fails when the video is **stale** — the manifest (`data/.generated/films.json`) hashes the storyboard, the article's date and the engine (painter, scenes, character, sound, fonts), so editing any of them without re-rendering is caught.
- **Edit a filmed article's numbers → update its storyboard → re-render.** `node brand-crew/skills/receipt-films/build.mjs <slug>` (or `--stale`) voices the lines (cached by text in `~/.cache/receipt-films`), paints the frames, mixes voice + effects + score, and writes `public/films/<slug>.mp4` (960x540 H.264 + AAC), `.vtt` captions and `-poster.webp`, and refreshes the manifest with the transcript. `--social --out=drafts/films` renders a 1280x720 cut for `/amplify`, not committed.
- **On the page:** `components/site/article-film.tsx`, in the article header, reads only the manifest (never `public/` via `fs`). Ambient by default — muted, looping, playing only while on screen; **sound on** restarts it with narration and captions and plays once. Never autoplays under `prefers-reduced-motion`, always has a pause, falls back to native controls with no JS. The transcript is on the page, in the article's JSON-LD as a `VideoObject`, and the film is `og:video`.
- **The character comes from a vendored skill.** `brand-crew/skills/claude-animation-base/` is John Heibel's Claude Animation Base (MIT, see its `VENDORED.md`): Clawd, 31 acted emotions, and a guide to timing and animation principles worth reading before any bespoke short. Its own renderer uses p5.brush on WebGL, which costs tens of seconds a frame with no GPU; `receipt-films/engine/paint.js` re-implements its drawing API in Canvas2D and loads `clawd.js` unchanged, at ~0.1 s a frame.
- **Budget:** ~1.2-1.5 MB per narrated film. Encoder settings were chosen by measurement (see `render.mjs` and `audio.py`); a texture or motion change that looks free can double every file.
...
- An article's Receipts film → `/film <slug>` (**receipt-films**) → `data/films/<slug>.json` + `public/films/<slug>.mp4`
- A bespoke painted short with Clawd → **claude-animation-base** (vendored; read its `ANIMATION_GUIDE.md`)
...
- `pnpm validate:films [--storyboards] [--only=slug,…]` — hold every Receipts storyboard to its article (numbers, quotes, label proximity, limits) and every committed film to its storyboard (hash, files, no orphans). `--storyboards` skips the render checks while authoring. See "Receipts films"
