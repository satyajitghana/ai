...
- **Budget:** ~40-60 KB per thumbnail; ~1.5-2.5 MB per film, about 2 MB on average. The dense media run over (stipple up to ~3.3 MB), because motion blur turns their dots and hatching into tones the encoder has to spend on. Encoder settings were chosen by measurement (see `render.mjs` and `audio.py`).
