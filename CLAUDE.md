...
- `pnpm validate` — `typecheck` + `validate:content` + `validate:mdx` + `validate:math` + `validate:placeholders` + `validate:links` + `validate:assets` + `check:models` + `check:repos`. **Run this after any content/data edit, before committing.**
