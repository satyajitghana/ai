...
- **You write a storyboard, not code.** `data/films/<slug>.json`; the contract — scene types, word limits, narration budget, styles, the host, truth rules — is `brand-crew/skills/explainer-films/SKILL.md`. Seven styles, each with its own ground, line, type, transition and score: watercolour, chalkboard, blueprint, neon, notebook, riso, pixel. Every film has its **own host**: a cat, dog, fox, bunny, capybara or fish with its own name, fur, ears, hat, glasses, outfit and prop, presenting big from the right of the frame, cropped by the bottom edge (no stage). Not Clawd.
...
- **Watercolour is real p5.brush, painted once and reused.** `engine/brushbake.js` paints each shape and line with the vendored p5.brush (`engine/vendor/`, MIT) on SwiftShader WebGL2, keeps the painting keyed by its geometry, and lays it down with `multiply`; fades and reveals reuse it. The host's outline is its traced silhouette inked in p5.brush charcoal every drawing. A watercolour sheet or film therefore takes minutes rather than seconds. Mesa's llvmpipe and lavapipe backends hang here and Bun has no WebGL, so SwiftShader is the path; `render.mjs` passes its flags.
