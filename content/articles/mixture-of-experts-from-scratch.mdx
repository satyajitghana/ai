...
<Video
  src="/articles/mixture-of-experts-from-scratch/moe-film"
  poster="/articles/mixture-of-experts-from-scratch/moe-film-poster.jpg"
  narrated
  captions="/articles/mixture-of-experts-from-scratch/moe-film.vtt"
  alt="A narrated hand-drawn film, 58 seconds, in eleven beats. Captions are available on the player. One tall orange slab stands for a dense feed-forward layer: three tokens cross it in turn, and for each one the whole slab lights red. The slab then separates into a stack of eight smaller slabs, each drawn as a little three-layer network of dots and edges. A token arrives at a hexagon-filled router on the left; eight dotted tracks run from the router's ports to the eight slabs, and a solid bar grows along each track as far as that expert's score. The two longest bars reach their slabs and sprout arrowheads; the other six retract and their slabs fade out. The two chosen slabs turn orange, their edges light red in a wave from left to right, and two red discs of different sizes leave them and merge into one. An ink blot then opens a chalk-on-navy blueprint: a grid of eight cells fills one by one, six of them empty again, and only the two that ran stay filled, with the stack of eight still drawn beside it. Back on paper, a blue token takes a different route and a different pair of slabs lights. Ten tokens flow through in sequence, each lighting two slabs, until every slab carries a small mark. Traffic then bends onto a few slabs: load bars grow to the right of the stack, one running long while others stay stubs and their slabs go pale. The scores tremble, the winners flicker between neighbours, and small chevrons press the long bars back and pull the short ones out until all eight sit near a dashed line. It closes on paper with ripples and the hand-lettered words 'sparse / by design'."
  caption="How one token gets routed, and why a model's parameter count and its per-token compute stop being the same number. Press play — it is narrated, and captioned. Drawn frame by frame in JavaScript on a 2D canvas at 12fps, voiced with Kokoro-82M on CPU, and cutting to a chalk-on-night blueprint for the memory-versus-compute beat. It is a drawn explanation, not a recording of a model: the eight experts, the two winners, the scores the router produces and the lopsided load bars are all illustrative. The counts match the tiny model built below — eight experts, top-2 routing — but no number on screen is measured. The measured ones are in the text: 4.52M parameters with about 1.4M active per token in tinymoe, and Mixtral 8×7B's 47B total against about 13B active. (Drawn with the vendored hand-drawn-canvas-animation skill, MIT, Alexey Fateev.)"
/>
