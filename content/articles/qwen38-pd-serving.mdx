...
5,000 total tok/s/GPU is about **549 generated tok/s/GPU**. If you are sizing a
...
&times; 549 generated tok/s/GPU over 2,560 concurrent users is 10.3 tok/s/user,
...
Same eight GPUs, same topology, MTP the only difference. Below concurrency ~400,
MTP is worth 1.6&times; and the post's summary is right that it *"drastically
improves performance."* Above it, the same flag costs **62% of throughput**.
...
**Reproducibility:** the strongest part. `vllm/vllm-openai:nightly-a9a17` (vLLM
...
<Falsifier claim="'Total token throughput' counts prompt tokens, so 5,000 tok/s/GPU is about 549 generated tok/s/GPU.">
At ISL/OSL 8192/1024 the output share is `1024/9216 = 11.1%`. The cross-check is
that 48 GPUs &times; 549 over 2,560 users gives 10.3 tok/s/user, within reach of
