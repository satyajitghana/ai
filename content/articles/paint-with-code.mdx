...
[Surya Narreddi and Cameron Franz](https://surya.website/rling-qwen-to-paint-with-code) trained a language model to paint by writing **p5.brush JavaScript** instead. The artefact is source code, so the picture is editable at the line level. That premise is a good one and it is not what makes the write-up worth reading. What makes it worth reading is that it is one of the few honest accounts of trying to run RL against a reward nobody can verify — and of the specific way that reward fell over.
...
**The 3× is a speed claim, and the interesting number is the ceiling.** "Reached the previous plateau three times faster and kept climbing" is exactly the right thing to report from a run in progress, and it is not the same as knowing where the second run stops. A curve that climbs past 0.65 might be finding real quality or might be finding the second rubric's own exploitable corner — the first rubric's reward also climbed, right up to the clip-art flower.
