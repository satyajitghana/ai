...
<Figure
  src="/articles/inkling-small/fig1.png"
  alt="A ten-axis radar chart comparing Inkling-Small against Inkling, DeepSeek V4 Flash, Gemini 3.5 Flash-Lite and GPT 5.6 Luna on a shared 0 to 100 scale. The axes are labelled with their category: HLE (no tools) and AIME 2026 for reasoning, SWE-bench Pro (Public) and Terminal Bench 2.1 for agentic coding, MCP Atlas (public) for general agentic, SimpleQA Verified and AA Omniscience for factuality, AudioMC for audio, MMMU Pro for vision, and IFBench for chat. The purple Inkling-Small polygon tracks the blue Inkling polygon almost exactly on most axes and sits slightly outside it on IFBench and Terminal Bench 2.1, but falls visibly inside it on SimpleQA Verified and AA Omniscience."
  caption="Small against big on one sheet: the two polygons are nearly coincident everywhere except the two factuality axes, where the purple one folds inward. That is the whole finding of this release, drawn rather than tabulated. (Thinking Machines Lab, Inkling-Small announcement post — the lab draws this chart in the page rather than publishing an image, so this is a capture of its own rendering.)"
/>
...
Two of those rows show up on the blog's own token-efficiency panel, which is the clearest single view of
both halves of the result:

<Figure
  src="/articles/inkling-small/fig2.png"
  alt="Four scatter panels — GDPval-AA v2 Elo, Tau-cubed Banking, AA-Briefcase Elo and CritPt — each plotting score against output tokens per task on a logarithmic axis, with a dashed Pareto frontier drawn through the non-dominated points. Inkling-Small is a purple dot, Inkling a blue dot, and the rest of the comparison set is grey. Inkling-Small scores above Inkling on GDPval-AA v2, AA-Briefcase and CritPt — at fewer tokens on the first two and at more tokens on CritPt — but sits well below it on Tau-cubed Banking, at roughly 15.5 percent against Inkling's 23.7 percent."
  caption="Score against output tokens per task, non-dominated frontier dashed in. Inkling-Small outscores its larger sibling on three of the four panels and does it on fewer tokens in two of them; the τ³-Banking panel is where it loses, by the margin the paragraph above quotes. (Thinking Machines Lab, Inkling-Small announcement post.)"
/>
...
RL](/articles/scaling-agentic-rl). Neither Thinking Machines source ships a figure as an image file:
the Hugging Face repository contains no images at all, and the announcement post draws its charts in
the page — inline SVG for the token-efficiency panels, client-side for the radar — rather than
publishing them as assets. The two static figures above are those charts reproduced as published;
the distillation and parameter-count diagrams are original illustrations of the mechanism, and there
is no paper here to reproduce a figure from.*
