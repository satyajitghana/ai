...
[handbook.modular.com](https://handbook.modular.com/) is 58 pages and 70,493 words
...
and GPU architecture, with four calculators and a dozen simulators built in.

<RepoCard repo="modular/llm-inference-handbook" />

It has been on my reading list since it appeared and I never wrote about it,
because a summary of a handbook is worth nothing. What is worth something is
checking it. This site has a lot of first-hand inference arithmetic lying around
— a decode-ceiling measurement on a 5090, a speculative-decoding sweep on
MI300X, a hybrid-attention KV budget recomputed from published geometry — and a
handbook is exactly the kind of document you can hold those against.
...
**Five of the handbook's formulas are correct formulas attached to the wrong
variable.** Not one of them is wrong as mathematics. Each one takes a quantity
whose name matches something the reader has to hand, and that something is a
different number. Leviathan's acceptance-length formula meets the acceptance
rate your server logs. The KV-cache shape meets `num_attention_heads`. The
weight footprint meets a nominal bit width. The tensor-parallel constraint meets
attention heads instead of KV heads. And they all land in the same place: the
calculator, the code block, the sizing rule — the parts a reader copies, rather
than the prose, which is mostly careful.
...
from a `bentoml/…` branch until #190 on 2026-07-29. From #192 onward the
branches are `modular/…`. The top two committers are BentoML's content lead and
BentoML's founder. **This is BentoML's handbook, and Modular acquired it.** The
neutrality is inherited, not authored.
...
<Figure
  ...
  caption="Three tokens drafted, two accepted, one rejected, one supplied by the target: three tokens committed for one verification pass. The ratio a serving stack logs from this round is accepted over proposed — two-thirds — and the chain stops at the first rejection, so the third draft was never really evaluated. (Modular LLM Inference Handbook, speculative decoding, Figure 1, Apache-2.0, rendered from static/img/diagrams/spec-decoding.svg at commit 0d64912 and flattened onto white.)"
/>
...
exactly 1.000 GiB. The paragraph is right, and 64 GiB divided by that is 59.6
...
GB**. Sixteen sequences instead of sixty. The sentence and the tool it cites as
its own working differ by a factor of four.

The factor is the GQA group size, and it is not small on anything modern. Here
is every preset in the handbook's own model list, at fp16:
...
vLLM says so in one line: `if self.use_mla: return 1  # When using MLA during
decode it becomes MQA`. At a 128K context the calculator claims 488 GiB where the
model holds 8.6.
...
The binding constraint is the KV heads, and vLLM says so in four lines
(`vllm/config/model.py`, `72675c4`):
...
input, output or combined, two pages after telling you always to check.

**Its one piece of roofline arithmetic is right, and I can check it against
...
The handbook asserts the compute-bound/memory-bound split ten times. It never
gives you the ratio that decides which side you are on. **"Arithmetic intensity"
appears once in 70,000 words**, in a passing clause about what tensor cores are
for. There is no roofline, no FLOPs-per-byte, no ridge point.

That gap is upstream of two of the errors above. Prefill is not compute-bound
...
- **The KV cache is decoupled from almost everything people couple it to.** Not
  ...
  dtype — nothing else.
- **A vendor handbook can be better than a neutral one, and this one is.** It is
  also, in part, better because it is not originally the vendor's, which is a
  fact you can only get from the git history.
...
<Falsifier claim="Modular acquired this handbook from BentoML, which is why its own product is only the third most-mentioned framework in it.">
**Reasoned**, from evidence that is strong but circumstantial: every merged PR
came from a `bentoml/…` branch until 2026-07-29 and from `modular/…` after
2026-08-03, the top two committers are BentoML people, and the cited
speculative-decoding benchmarks are BentoML's. I have not seen an acquisition
announcement and did not look for one. A licensing deal, a hire, or an
open-source donation would produce the same git history and a different story
about why the coverage is even-handed.
</Falsifier>
...
<Falsifier claim="Arithmetic intensity is absent from the handbook, and that absence is upstream of its memory-model errors.">
**Reasoned**, and the causal half is the weak half. The count is a fact — one
occurrence in 70,000 words, in a clause about tensor cores. The claim that a
roofline section would have caught the percentage-overhead model is a story I
find persuasive and cannot demonstrate. It is entirely possible to reason
correctly about the ridge point and still ship a calculator that multiplies the
cache allowance by the weight precision, because the two live in different parts
of the brain and, here, in different chapters written months apart.
</Falsifier>
...
Apache-2.0), 58 pages and 70,493 words of `docs/`, plus the four calculator
