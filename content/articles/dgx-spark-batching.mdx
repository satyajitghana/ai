...
<Figure
  src="/articles/dgx-spark-batching/fig1.png"
  alt="Diagram titled Persistent Batch in MRV2. On the left, a five-row grid labelled Block Table State holds three coloured rows, labelled Req C, Req B and Req D, separated by two empty white rows. A large grey arrow labelled Gather, fed from below by a req_order list reading B, C, D, points right to a three-row grid labelled Input Block Table containing Req B, Req C and Req D in that order."
  caption="What rebuilding the batch every step looks like in vLLM's own design docs — the mechanism this section describes, not the benchmark above it, whose source ships no figures. vLLM keeps one persistent row per live request and gathers the step's actual input from only the rows that are alive; the gaps left by requests that finished are simply not gathered (vllm-project/vllm, docs/design/model_runner_v2.md)."
/>
...
<Figure
  src="/articles/dgx-spark-batching/fig2.png"
  alt="Diagram of vLLM's KV cache manager. A row of blue boxes labelled Block Pool (list), each carrying ID, Hash, Prev and Next fields, is chained into a doubly linked list running from ID 0 to ID N. A green box labelled Free Block Queue holds Head and Tail pointers, with curved arrows into the list. Below sit two small tables: Cache Blocks, mapping a hash key to block IDs, and Request Blocks, mapping a request ID to block IDs."
  caption="The shared pool itself. Every KV block is allocated once into a block pool and threaded onto a free queue, with a separate map from each request to the block IDs it holds — which is how a finished request hands its memory straight to whoever is next instead of going back to an allocator (vllm-project/vllm, docs/design/prefix_caching.md)."
/>
