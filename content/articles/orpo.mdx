...
description: "ORPO appends a log-odds-ratio term to the SFT loss and drops the reference model. The maths is worth learning — the odds ratio is chosen for its gradient behaviour, and derived on the page here, the argument holds up. The compute story is oversold: the frozen reference is 13.49 GiB against 107.91 GiB of trainable state at Mistral-7B, and deleting it cuts 25% of per-step FLOPs, not half — and TRL's DPO already keeps no second copy under LoRA. The win the paper never measured is GSM8K, where ORPO has the best mean delta of any preference objective, +2.50 against DPO's −8.27. Also: a sign error in Equation 8, and why ORPOTrainer now lives in trl.experimental."
date: 2026-09-19
interest: 5
helpful: 5
featured: true
