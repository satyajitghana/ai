...
description: "ORPO appends a log-odds-ratio term to the SFT loss and drops the reference model. The maths is worth learning — the odds ratio is chosen for its gradient behaviour and the argument holds up. The compute story is oversold: the frozen reference is 13.49 GiB against 107.91 GiB of trainable state at Mistral-7B, and removing it cuts 25% of per-step FLOPs, not half. The win the paper never measured is GSM8K, where ORPO is the only preference objective that goes up."
date: 2026-09-19
interest: 5
helpful: 5
featured: true
...
Three of those are true and one is a number that nobody has written down. This
piece writes it down, and derives the objective properly on the way, because the
interesting part of [ORPO](https://arxiv.org/abs/2403.07691) is not that it
removes a stage — plenty of methods do — but *which ratio it contrasts with and
why*, which is an argument about gradients that the paper makes carefully and
almost nobody repeats.

Short version of where this lands: the odds ratio is a genuinely good idea and
the derivation rewards reading. The reference-model saving is real and is about
11% of model-state memory and 25% of per-step arithmetic. And ORPO's best result
is in a table the paper never printed.
...
probability, a 400-token response would sit around $e^{-400}$, $1 - P$ would be
1 to 170 decimal places, and $\mathbf{odds} \equiv P$ exactly. The odds ratio
...
Mistral-ORPO-β beats Zephyr-β by 1.57 raw points, and **17 head-to-head wins out
of 805**. The standard errors are ±0.99 and ±0.96, so the difference is about 1.1
standard errors of its own difference — real, directionally consistent, and not
separable from noise on this benchmark alone.
...
Against SFT-only and against PPO, ORPO wins decisively everywhere. Against DPO it
is 41.7, 49.4, 48.8 and 50.5 below 1B — three coin flips and one loss — and wins
at 1.3B. The paper's framing, *"the win rate over DPO was correlated to the
model's size"*, is a three-point trend in each dataset and is offered as the
...
essentially ties in the fourth (14.7 against 15.1). It beats CPO consistently,
which is the head-to-head §7.1 predicted and is a point in the odds ratio's
favour. And it loses to SimPO everywhere, which is SimPO's paper, so read it with
that in mind.
...
ORPO is the only objective in the table that takes GSM8K **up** — 28.13 to
42.15, +14.02 over the checkpoint it started from — while DPO takes it down to
21.76. It also posts the highest MMLU of any row, 63.20, above the SFT model it
began from, in a column where DPO loses 1.6 points. SimPO's own explanation is
the obvious one and it is the argument for ORPO's whole design: *"except for
ORPO, almost all approaches lead to consistent drops in one or more settings. We
hypothesize that ORPO retains performance largely due to its supervised
fine-tuning loss for regulation."*

That is the strongest case for the method, and it is nearly the opposite of the
case the paper makes. ORPO's selling point is not that it deletes a stage; it is
that keeping the SFT term *inside* the preference objective, active on every
step, stops preference training from eating the model's arithmetic. Alignment tax
paid in GSM8K points is a well-known cost of DPO. ORPO does not pay it — and the
paper reports AlpacaEval, MT-Bench and IFEval, all three LLM-judged or
instruction-format benchmarks, and not one multiple-choice or exact-match task
...
<Falsifier claim="ORPO's real advantage is downstream-task retention — GSM8K and MMLU — not preference win rates.">
One block of one appendix table, Mistral-Base only, and the ORPO column there was trained from an SFT checkpoint rather than from base. Reproducing it on Llama-3-Base and on a second preference set would make it solid; a single setting where ORPO drops GSM8K below its SFT starting point would make it much weaker. KTO is close behind at 38.51 in the same table, so "ORPO alone retains math" is also too strong — "objectives with an SFT term retain math" is the defensible version.
</Falsifier>
