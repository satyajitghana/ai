...
744B total parameters, 40B active per token — a mixture-of-experts on a
256-expert backbone. That 744B is the backbone-only count, and it is worth
pinning down because the corpus and the wider write-ups disagree on it: reading
every shard header gives **753,329,940,480** parameters, matching Hugging Face's
own element count to the last digit, of which the 78-layer backbone is
**743,377,019,904** and the rest is the multi-token-prediction layer. So 744B and
753B are the same model counted with and without the MTP module, not two
different models — the shard-by-shard audit is in
[GLM-5.3 is the same weights](/articles/glm-5-3). Attention is **DeepSeek Sparse Attention (DSA)**: Multi-head
...
<Figure
  ...
  caption="Pairwise overlap of each layer's selected tokens. Neighbouring layers pick nearly identical sets — so recomputing the indexer for each is wasted work (IndexCache paper, Figure 4)."
/>
...
<Figure
  ...
  caption="The mechanism: an F-layer computes the indices and caches them; the following S-layers reuse the cache, skipping the indexer entirely (IndexCache paper, Figure 2)."
/>
...
<Figure
  ...
  caption="Speculative decoding with IndexShare + KVShare — keeping the draft and verify passes consistent (Z.ai)."
/>
...
<Figure
  ...
  caption="Long-horizon benchmarks (FrontierSWE, PostTrainBench, SWE-Marathon) — the gap to the frontier is small (Z.ai)."
/>
...
<Figure
  ...
  caption="Effort vs token budget — Max trades more tokens for more capability on hard tasks (Z.ai)."
/>
...
<Figure
  ...
  caption="The IndexShare payoff at serving time: the throughput edge widens as context approaches 1M tokens (Z.ai)."
/>
