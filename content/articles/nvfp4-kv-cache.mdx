---
title: "SGLang's 4-bit KV cache: the 56% is exact, the 1.78x is untested"
...
The first is exact and better than it looks. The second is arithmetic that the
...
NVFP4's advantage shrinks. SGLang's `fp8_e4m3` baseline is per-tensor, so the
comparison here is fair.

For the model actually benchmarked, Qwen3.8-27B, this is 14 KiB per token:
...
would shrink four times as much state.
...
and the four figures published with it. Benchmark commit `c8b56b1`, AgentX harness
