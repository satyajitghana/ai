...
arguments, is where structured tool use is genuinely hard, and **30%** of those cases still slip. The
...
effective **under 2.5 bits per parameter**. BTL publishes the byte ledger behind that figure:

<Figure
  src="/articles/btl-3/fig1.png"
  alt="A single stacked horizontal bar titled 'Where the 8.38B GGUF tensor bytes went', split into AVQ2 5.498 GB (65.60%), Affine INT4 0.919 GB (10.97%), BF16 islands 1.206 GB (14.39%), Vocabulary 0.668 GB (7.97%), Small state 0.058 GB (0.69%) and Behavior adapter 0.032 GB (0.39%). Footnote: tensor payloads 8,381,262,032 bytes, final GGUF 8,392,369,600 bytes."
  caption="How the 8.39 GB is spent: two thirds in AVQ2 vector codes, a seventh in the BF16 islands held back at full precision, and 0.39% in the rank-8 behaviour adapter that repairs what the rest broke (BTL's compression paper, Badtheorylabs/BTL-3)."
/>

The claimed cost of that compression is measured on a
...
It is also an aggregate, and BTL's own chart of it says so in the subtitle:

<Figure
  src="/articles/btl-3/fig2.png"
  alt="A horizontal bar chart titled 'Fresh sealed tool-behavior retention', subtitled 'Conditional on teacher-correct turns; the aggregate hides one weak category'. Single, Parallel, Sequential and Abstention each show 100% on n=20 teacher-correct turns; Parallel-multiple shows 30% in red on n=10; the Overall bar is 92.2% on n=90. Footnote: 83 of 90 teacher-correct turns retained, absolute score 83 out of 100."
  caption="The 92.2% broken out by category. Four categories retain everything the full model got right; parallel-multiple retains 30% of ten turns, and that single row is the whole gap (BTL's compression paper, Badtheorylabs/BTL-3)."
/>

That is the same weak spot the full model shows on BFCL — parallel-multiple, at 70.0% there — only
sharper, and on ten turns rather than a benchmark suite. Read the two together and the picture is
consistent: composing several calls with several arguments each is where this model is thinnest, and
compression takes what little slack was left.
...
private, conditional gate, not a public benchmark — one whose aggregate hides a **30%** row. The model
card ships **no figures or diagrams**, so the loop diagram above is my own reconstruction of the
described behaviour; the two Compact charts are BTL's, from the compression paper in its runtime repo.
No training-data disclosure is provided.
</Callout>
...
[runtime source](https://github.com/Badtheorylabs/BTL-3), whose
`papers/behavior-before-perplexity/` directory carries BTL's compression paper and the two Compact
charts reproduced above. The model card itself ships no figures, so the agent-loop diagram is mine;
all benchmark numbers are Bad Theory Labs' self-reported values.*
