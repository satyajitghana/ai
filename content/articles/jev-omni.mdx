...
- **Read the four latency numbers, not the one.** 26 ms for an image and 504 ms
  for sixteen of them is the same model doing the same arithmetic on sixteen
  times the visual tokens, for nineteen times the latency. Quote the one that
  matches your input.
