...
description: "RETRACTED BY THE LAB, the day after this was published. Basalt Labs described Monolith 1.0 as a 1.57T-parameter open Mixture-of-Experts with 49.5B active per token — top-2 routing over 128 experts plus one shared, a two-stage YaRN stretch to a 1M-token context, and an SFT/DPO/RLVR reasoning-RL recipe. The Hugging Face repo now holds a 436-byte README saying the weights are gone and the public model was an inflated Qwen 2.5 7B Instruct. Kept as a record of what was claimed: the mechanisms are real, the model was not."
date: 2026-07-17
updated: 2026-09-19
interest: 3
helpful: 3
...
**Retracted by Basalt Labs on 18 July 2026 — the day after this was published. Read the
rest as a record of what was claimed, not of what exists.**

The Hugging Face repository now holds a single 436-byte README and nothing else. In full, it
says the experiment has concluded, that *"the model intended for the public is an inflated
version of the original Qwen 2.5 7B Instruct model"*, and that *"the weights of the ~3TB model
have been removed"*. Every specification below — 1.572T parameters, 49.5B active, 128 experts,
60T training tokens, the 1.8e25 FLOP budget — was read from a model card that no longer exists,
describing weights that are no longer downloadable, for a model its own authors now say was
something else. The `<ModelCard>` panel below reports the repository as it stands today: one
file, 436 bytes, zero downloads.

I am not deleting this. The mechanisms it walks through — top-2 MoE routing, two-stage YaRN,
self-speculative decoding — are real techniques and the explanations of them stand on their own.
What does not stand is that any of it describes a model that shipped. The scepticism in "The
benchmarks" below was aimed at self-reported scores; the retraction went further than that, and
faster.
</Callout>
...
Compared against GPT-5.4, Claude Opus 4.6, Gemini 3.1 Pro, and Kimi K2.6 — **every bar below,
including the comparators, is Basalt's own figure from its own table**, so the chart is internally
consistent with the claim it is making and with nothing else:
...
<BenchBars
  ...
  bars={[
    ...
    { label: "GPT-5.4", value: 40.0 },
    { label: "Claude Opus 4.6", value: 40.0 },
    { label: "Gemini 3.1 Pro", value: 44.4 },
    { label: "Kimi K2.6", value: 34.7 },
  ]}
/>
...
<BenchBars
  ...
  bars={[
    ...
    { label: "GPT-5.4", value: 99.2 },
    { label: "Claude Opus 4.6", value: 96.7 },
    { label: "Gemini 3.1 Pro", value: 98.3 },
...
<BenchBars
  ...
  bars={[
    ...
    { label: "GPT-5.4", value: 92.8 },
    { label: "Claude Opus 4.6", value: 91.3 },
    { label: "Gemini 3.1 Pro", value: 94.3 },
    { label: "Kimi K2.6", value: 90.5 },
  ]}
/>
...
<BenchBars
  ...
  bars={[
    ...
    { label: "GPT-5.4", value: 90.2 },
    { label: "Claude Opus 4.6", value: 89.7 },
    { label: "Gemini 3.1 Pro", value: 90.8 },
    { label: "Kimi K2.6", value: 88.9 },
  ]}
/>
