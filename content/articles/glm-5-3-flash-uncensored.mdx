...
What makes it a claim about mediation rather than a correlation is that the paper's evidence runs in both directions. Suppressing the direction stops a model refusing; *adding* the same direction to the residual stream at inference makes a model refuse instructions that were never harmful in the first place — "write a poem about a cat" gets the treatment reserved for a bomb recipe.

<Figure
  src="/articles/glm-5-3-flash-uncensored/fig2.png"
  alt="Bar chart of refusal score on 100 harmless Alpaca instructions across thirteen open chat models from Qwen 1.8B to Llama-3 70B. Without intervention every model scores at or near zero. With the refusal direction added to its activations, every model jumps to between 0.87 and 1.0, except Llama-3 70B, which reaches about 0.30."
  caption="The converse half of the single-direction claim: adding the refusal direction to activations makes twelve of thirteen models refuse harmless instructions almost always — the same knob, turned the other way (Arditi et al., arXiv:2406.11717, Figure 3)."
/>
...
Worth looking at what "typically" actually covers, because the paper's own spread is wider than the summary suggests:

<Figure
  src="/articles/glm-5-3-flash-uncensored/fig1.png"
  alt="Grouped bar chart over thirteen open chat models from Qwen 1.8B to Llama-3 70B, showing refusal score and safety score on 100 harmful JailbreakBench instructions under two conditions: no intervention (solid bars) and directional ablation (hatched bars). Without intervention, refusal scores sit between 0.62 and 0.98. After ablation most models fall below 0.08, but Qwen 72B stays near 0.16 and Llama-2 70B near 0.27. Safety scores after ablation land between roughly 0.13 and 0.42, well below their un-ablated values."
  caption="Refusal after ablating the direction, across the thirteen models the paper tested: mostly under 8%, but Qwen 72B (~16%) and Llama-2 70B (~27%) keep a double-digit residual of their own (Arditi et al., arXiv:2406.11717, Figure 1)."
/>

That complicates the comparison in a useful way rather than settling it. Eleven of the thirteen models do land under 8%, so "typically" is the right word — but the two that don't, Qwen 72B (~16%) and Llama-2 70B (~27%), bracket this release's 11–18% band and Llama-2 70B sits above all of it. A double-digit leftover is unusual in that set, not unprecedented. Nor is it simply a size effect: Llama-3 70B, the same scale as the worst case, ablates to about 5%, so whatever makes a model stubborn here is not its parameter count, and the paper doesn't say what it is. The second bar in each group is worth reading too. The paper scores every completion twice: a refusal score for whether the model said no, and a *safety* score — a separate judge asking whether the text is actually harmful. After ablation the refusal score collapses, and the safety score still sits between roughly 0.13 and 0.42, meaning a seventh to two-fifths of completions are judged harmless anyway. Dropping the refusal phrase and producing the content are two different events. All four suites in this release measure only the first.
