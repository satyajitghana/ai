...
description: "WindTunnel's newest board has a decision model at rank one — Jev picks the tool, Mercury 2.5 writes the arguments, 49 of 49 tasks solved over WebMCP. The published multipliers, 112x and 245x against GPT-6 Astra, are ratios of median model cost, and both factor cleanly into an interface term and a price term that the same board lets you separate. Doing that separation moves the interesting number from 245 to 15. Along the way: the cheap fast model turns out to be 78% of the bill, and the 18% saving that came with doubled accuracy is entirely the cost of failed attempts."
date: 2026-09-19
interest: 5
helpful: 5
featured: true
...
Two articles ago this site argued, from TypeSafe's docs, that Jev must be a
per-option scalar scorer. Then an independent benchmark put it at
...
seen someone ship a real system into that constraint, measure it against nine
alternatives on the same tasks, and publish everything needed to check the
arithmetic. Jev + Mercury 2.5 over WebMCP solves **49 of 49 tasks** and lands at
...
- id: lh-4
  ...
  predicate: { type: answer, contains: [webmcp] }
```
...
```js
export const PRICES = [                      // $ per million: in, out, cached read, cache write
...
million (input / cached / output). Jev is **$0.042 per million for every token
type, with output free** — a consequence of the shape of the thing, since its
output is a choice id and a probability vector, not a stream.
...
price table. Both reproduce to the cent, so the reported column really is the
stated formula:
...
that range — the priciest model in the field on its heaviest interface. Against
the best conventional WebMCP row, GPT-5.6 Luna at 49/49 and 146/147, the gap is
**2.3x**.
</Callout>
...
One more thing the transcripts settle. Mercury is sold at 1,000+ tokens/second,
which is a decode rate. End to end in this harness its **median call is 614 ms**
for a median of about 50–60 output tokens — roughly 65 tok/s wall-clock, because
a sixty-token JSON object is all prefill and round trip. Jev's median call is
...
<Figure
  ...
  caption="The other side of the comparison: without WebMCP the menu is the page's own controls, and the operation and target heads are answered speculatively in one request. BLOCKED is one of the six operations on offer — and in the benchmark it is the single most common way the page setup ends (browser-use/jev-ultrafast, docs/inspector.png)."
/>
...
Four things here were published against their author's interest.
...
hashes and 294 redacted attempt transcripts. Everything below runs without an
API key, and all of it passed on first go:
