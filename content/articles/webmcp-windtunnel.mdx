...
```
ea-7   step 5   0 → 4 tools   28 ms  1 poll   ready-after-wait   (attempt failed anyway)
hev-8  step 2   0 → 1 tool    85 ms  3 polls  ready-after-wait   pass
hev-8  step 2   0 → 1 tool    83 ms  3 polls  ready-after-wait   pass
hev-8  step 2   0 → 1 tool    79 ms  3 polls  ready-after-wait   pass
```

Three of the four are the same task, all three at the same step, and all three
passed. Hi.Events is the application the shared arm's own comment singles out —
*"Heavy SSR apps (hi-events) hydrate well past 5s"* — and step 2 is immediately
after a navigation.
...
`__finish__` and `__abstain__`, and ask Jev to choose between reporting an answer
it does not have and giving up, two steps into an eight-step journey. Both are
scored failures.
...
Reasoned, and the cheapest thing on this list to settle: re-run `hev-8` three times with the readiness timeout set to 0, which the adapter takes as a parameter. If it still passes 3/3, the empty list at step 2 was recoverable some other way and this caveat is wrong. Roughly a dollar of API credit and ten minutes of Docker.
</Falsifier>
