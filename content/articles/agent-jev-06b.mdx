...
*bit-identical* between the two orders. This is not a training result and it is not
a number that got small: nothing in this model has anywhere to put a position
prior, so the only thing left to measure is the width of the float.
...
at the level of the arithmetic rather than at the level of the answer. What makes it worth
...
content perturbation with the size held fixed: median SUB_MEDIAN logits, worst
SUB_MAX, the same order of magnitude as before. It is the contents, not the count.

And the proportionate qualifier, because a tenth of a logit is not a lot: on this
sample, removing a losing option changed the winner in DROP_WINNER drops. The channel
...
probability, on one three-option question, in a float with eight mantissa bits. A
difference of the size I measured could not have shown up there. It is a smaller
number on a coarser instrument.
...
**The answer never changed. 0 of RUN1_N**, Choice and Score questions with four or
...
**Denominators.** One benchmark, one CPU, float32. The end-to-end run is small — RUN1_N questions — because a 0.6B model in float32 on four cores shared with whatever else is on the box is a couple of minutes per reversed pair, and the permutation sweep is large only because it reuses one backbone pass each. What makes me willing to state the null flatly anyway is that it is not a statistical null. A transformer with no positional embeddings is a set function; the backbone deltas are exactly zero; the residual is at the float's last bit rather than at any scale a prior could live at. If a permutation produced a flip, the explanation would have to be a bug, not a bias. What I have *not* run is bf16, which is the dtype the GPU service uses and where the repo's own `permutation_invariance_kl` guard says it was worried.
</Callout>
...
learn the teacher. The hygiene on both sides is genuinely good — better than the
