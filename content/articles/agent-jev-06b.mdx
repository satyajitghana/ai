...
description: "Four articles on this site have described a fork: isolate a decision model's options and you get exact order-invariance but no option can refer to another; put them in one sequence and a relational question becomes expressible but position acquires a prior worth +1.71 logits. AgentJev-0.6B is arranged a third way — one cached prefix, one private branch per candidate, and a scoring head with no positional embeddings — and it pays neither price. I downloaded the weights and ran the reversal test on CPU: the answer changes on 0 of 96 questions, the backbone vectors are bit-identical, and the largest movement anywhere is 4.8e-07 logits. It is the first model in this family measured to be exactly order-invariant, on a week when the hosted one was measured flipping 12 of 100. Also: 598,418,689 parameters off the safetensors header, the 92.4% that buys 2.04x, the reliability diagram neither release drew, and a headline of 79.25% on a benchmark whose own card puts saturation at 73.5%."
date: 2026-09-22
interest: 5
helpful: 5
featured: true
...
list. A per-option scalar scorer must score exactly zero on that test by
construction; a vocabulary readout cannot.
...
**Zero flips, and the largest movement anywhere is RUN2_WORST logits** — a few
units in the last place of a float32. Every candidate's backbone vector is
*bit-identical* between the two orders. This is not a training result and it does
not degrade with scale or cardinality, because nothing in the model has a place to
put a position prior.

That is a real result, and it is the first time anything in this category has been
measured at exact order-invariance rather than asserted to it. What makes it worth
an article rather than a footnote is *how* it is arranged, because it is not the
arrangement that was supposed to buy this — and because the same model's score for
an option does depend, heavily, on what the other options say.
...
Three sentences, three properties. The prefix — `[STATE] …\n[QUESTION] …` — is
encoded once and cached. Each candidate is a private branch off that cache, so no
candidate is in any other candidate's context. And every branch starts at the same
position index, because they all continue the same prefix, so candidate 7 and
candidate 40 are encoded at *identical* rotary positions. There is no slot to have
a prior about.
...
The scoring path is eight lines:

```python
# agent-jev · agentjev/model.py @ b66a8ce — AgentJevModel._score
...
`proj_out(enc)` is the correction that knowing the rest of the set buys you. Set
the set-encoder's output to zero and you have `open-jev-deberta` exactly — a
learned scalar head over an isolated encoding. The whole of the departure from the
per-option family is one additive term.
...
before anything knows the set. AgentJev's move by a median of COMP_MEDIAN logits
and by as much as COMP_MAX.

<SetVsOrder />

Six decades separate the two perturbations. That is the article in one picture: the
...
measured on CPU is float32, and RUN2_WORST is what that costs. The repository's own
check ran on GPU under bf16 autocast and reported exactly `0.0`, on one
three-option question, which is a smaller number on a weaker test.

## The measurement, and how I got the number
...
**The answer never changed. 0 of RUN1_N**, all Choice questions with 4 or 5 options.
On the same test the openjev readout changes its mind on 27.8% and the hosted model
on 12%.
...
realigned. RUN2_N questions, **RUN2_PERMS permutations**, zero answer changes, and
...
**Denominators.** One benchmark, one CPU, float32. The end-to-end run is small — RUN1_N questions — because a 0.6B model in float32 on four contended cores is about a minute per reversed pair, and the permutation sweep is large only because it reuses one backbone pass each. What makes me willing to state the null flatly anyway is that it is not a statistical null. A transformer with no positional embeddings is a set function; the backbone deltas are exactly zero; the residual is at the float's last bit rather than at any scale a prior could live at. If a permutation produced a flip, the explanation would have to be a bug, not a bias. What I have *not* run is bf16, which is the dtype the GPU service uses and where the repo's own `permutation_invariance_kl` guard says it was worried.
</Callout>
...
2,368,769 parameters — 0.4% of the checkpoint — and the order-invariance property
specifically costs 1,579,520 of them. For scale, the
...
This corpus has now found an option count welded into an export twice: `N=25` in
an ONNX graph and `K=32` in a Core ML bundle. A third would make it a pattern, so
I went looking.
...
TypeSafe's own Choice takes. Even the celebrated 2,048-token context is
...
independent paths. That threshold is why the repo's own 255-candidate smoke test
...
from four workflows, evaluated on 400 held-out cases from the same four workflows,
*should* learn the teacher. The split hygiene is genuinely good: split by case id,
120 dev and 120 calibration cases reserved out of the 1,200, checkpoint chosen on
development soft cross-entropy before the test split was opened, seed and dataset
revision pinned. It is a reason to read +2.25 points over Laya as what it is, which
...
AgentJev publishes more than anyone in this category except `kev`. Expected
calibration error, sum-of-candidates Brier, soft cross-entropy and score MAE, per
primitive and per workflow, before and after temperature scaling, for its own
checkpoint *and* for the Laya checkpoint it re-measured, with all 2,000
per-question probability vectors committed. The temperature is one positive scalar
per primitive, fitted on 180 boolean, 180 choice and 240 score questions from
cases held out for exactly that and never used for anything else. That is the
protocol the RLCD piece asked for, done without being asked.
...
README I have read this month: Phase 5 was abandoned because *"policy value did not
improve and the stall judgment collapsed"*, the weights loaded are *"not a random
head or a 27B pretending to be Jev"*, and the unimplemented `TreeEncoder` is
explicitly *"not falsely claimed as a completed training speedup"* — which is true,
and which I only noticed because they said it.
...
nothing, composition carries COMP_MEDIAN logits at the median, and both properties
...
RUN2_PERMS permutations, with bit-identical backbone vectors and residuals at
...
Zero flips across RUN2_PERMS permutations of RUN2_N questions, worst movement RUN2_WORST logits, in float32 at cardinality 4 and 5. Two things would break it. Cardinality: run the sweep at 16, 64 and 255 options, where the set encoder's attention has far more terms to sum in a different order. Precision: run it under `torch.autocast(bfloat16)`, which is what the GPU service does and which has eight mantissa bits instead of twenty-three — the repository's own `permutation_invariance_kl` term exists because someone was worried about exactly that. If flips appear and grow with the option count, something in the serving path sees the ordering, and the first place I would look is the `common_prefix` scan: it folds any shared leading tokens of the candidate text into the cached prefix, so how much gets cached depends on which candidate happens to be first.
</Falsifier>

<Falsifier claim="The cross-candidate channel is wide enough to answer a relational question.">
I have shown the channel exists — dropping an option moves a surviving option's raw logit by a median of COMP_MEDIAN logits, where a per-option scorer would move it by exactly zero — and I have not shown it carries a *reference*. Those are different claims and the gap between them is the most interesting open question in this category, because the 0/100 on relational choice was supposed to have settled it architecturally. Build the "ship to the address in option D" case from the jev-scores-zero piece at n = 100, in AgentJev's own request schema, and score it. A per-option scorer gets 0 by construction. If AgentJev also gets 0, the channel is real and too narrow to carry a pointer — a 256-number summary of an option is not its text — which would be the most informative negative result available here. If it scores materially above chance, the fork this site has been describing for four articles is not a fork at all.
</Falsifier>
