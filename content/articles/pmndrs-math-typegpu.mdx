...
`software-mansion/TypeGPU` at `b819e00` (26 September) and read both. For the comparisons I read
gl-matrix (`6f96d57`, 3.4.4), wgpu-matrix (`3dba901`, 3.4.2) and three.js's `src/math` (r186) at
their current heads. I ran none of their code. I did run my own: esbuild bundles of math's source,
to see what tree-shaking keeps, and a two-bone IK solver written two ways, which is the widget
further down.
...
wgpu-matrix puts the destination last and optional: convenient, and a hidden allocation every time it
is left off. It also lays `mat3` out as WGSL does, with a padding float per column. three.js is
object-oriented, and it uses the same module-scratch trick internally
(`const _vector = /*@__PURE__*/ new Vector3()`). math's SKILL file tells agents to marshal three.js
objects through `toArray(scratch)` and `fromArray`, and warns that `toArray()` called bare allocates.
...
combination of argument types gets its own WGSL overload. That has a sharp edge the docs flag: a
literal that passes `Number.isInteger` is an integer, so `const bar = 1.0` generates `1i`. Wrapping a
function in a shell, `tgpu.fn([d.f32, d.f32], d.vec2f)(…)`, pins the signature. Operators such as `+` on
vectors need a companion tool, tsover, and the build plugin turns them into calls like
`__tsover_add`; without it you call `std.add` and friends.
...
That matters when IK meets terrain. A foot's IK target is a point on the ground. If the ground is
displaced in a vertex shader, the CPU needs the same height at the same point, or the foot floats.
**Reasoned:** one `'use gpu'` height function, called from the solver in JavaScript and from the
shader, is the cleanest way to guarantee that, to within the f32-versus-f64 difference TypeGPU's casts
narrow but do not close. A height computed with math on the CPU and uploaded works too, as long as
the shader samples the same buffer instead of recomputing.

## What it adds up to

math is a disciplined library: a gl-matrix API on plain arrays of doubles, with the caller owning
every byte. It comes with a benchmark suite that gates regressions statistically and snapshots
outputs. "Allocation-free" holds for the core, with three rotations that still allocate in 0.1.0.
"Tree-shakable" holds per namespace under esbuild. "Most optimized" is not something the repository
measures: every benchmark compares math with math, and none of the results are committed.

TypeGPU gives WGSL types a TypeScript shape and lets one function body serve both JavaScript and the
GPU, with value semantics on both sides. The two meet at the buffer, where TypeGPU takes math's plain
arrays as they are. Where they differ is the right place to differ: out-parameters where a garbage
collector lives, value types where it does not.

For more of this corner of the site: [Voxel Musou's crowd](/articles/voxel-musou#three-hundred-is-a-pool-size)
is the same allocate-once discipline on three.js, a fixed pool of soldier slots in typed arrays.
[OpenDLSS-NR's WebGPU port](/articles/opendlss-nr#what-same-output-is-measured-to-mean) checks a
JavaScript oracle against its WGSL, the CPU/GPU agreement problem from the other end. And
[the tiny browser models](/articles/tiny-browser-models) are what WebGPU compute looks like when
the payload is a neural network rather than a mesh.
