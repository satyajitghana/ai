...
<Callout type="note">
**Updated 2026-08-06.** The first version of this was written against a shallow clone, which is a
worse instrument than I gave it credit for. This revision is built from a full clone — 4,473 commits
of real history — and adds four things that were missing: the typed host-request bridge sitting
behind the single tool, how MCP servers are wired in without adding one, the two independent memory
systems a persistent kernel creates, and what runs when nobody is attached. It also replaces the
maturity section outright; the previous one said I could not audit the commit history, and now I
can.
</Callout>
...
The last piece the first version skipped: Prime Agent is built for sessions with no human in front
of them. Sessions live in resident daemon worker processes, so closing the terminal detaches a
...
This is the section the first version got wrong, and the fix is more interesting than the error.

I originally wrote that I could not audit the commit history because the clone I read from was
shallow — one commit visible — and declined to claim anything about the project's age. That was the
right call given the instrument, and the wrong instrument. A full clone answers the question
completely, and the answer is not what either a shallow clone or the changelog suggests.
...
The changelog backs that up rather than contradicting it. Since the first version of this article,
`0.6.1` and `0.7.0` have both landed, and `0.7.0`'s single breaking change is instructive: agent
