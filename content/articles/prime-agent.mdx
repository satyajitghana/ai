...
Those four kinds are not a product decision Prime Intellect made alone — they are the
paper's own decomposition. The Continual Harness formalizes harness state as
ℋ = (p, 𝒢, 𝒦, ℳ) — prompt, sub-agents, skills, memory — with a Refiner that reads a
window of the trajectory every F steps and emits per-component edits
Δ = (Δp, Δ𝒢, Δ𝒦, Δℳ), applied as ℋ ← ℋ ⊕ Δ. Prime Agent's `/refine` is the left panel
of this, shipped as a feature:

<Figure
  src="/articles/prime-agent/fig1.png"
  alt="Two panels. Panel (a), 'Harness refinement within one episode', shows an inner loop: an Agent reads the observation, the harness state and the trajectory and emits an action to the environment; below it, harness state is drawn as four boxes — prompt (policy), sub-agents, skills, and memory — each holding a small example entry; an orange Refiner box at the bottom reads a window of the trajectory every F steps after a warmup and emits per-component edits that are applied back into the four boxes. Panel (b), 'Co-learning across iterations', shows an outer loop of reset-free rollouts across successive iterations feeding a process reward model, a teacher relabelling low-reward steps, and a soft supervised fine-tuning step that updates the policy weights."
  caption="The paper's own shape for the idea: harness state is (prompt, sub-agents, skills, memory), and a Refiner reads a trajectory window and emits per-component edits applied as ℋ ← ℋ ⊕ Δ (left). The right panel — training the model weights alongside the harness — is the half Prime Agent does not attempt (Continual Harness, arXiv:2605.09998, Figure 2)."
/>

That split is the useful thing to carry away from the figure. Prime Agent implements the
inner loop faithfully — same four components, same edit-not-rewrite discipline, same
trajectory-as-evidence rule — and stops there. The outer loop, where the harness's own
trajectories become training data for the model's weights, needs a training stack a
coding agent does not have. What Prime Agent calls self-improvement is the left panel
only, and the paper is explicit that the two halves are different mechanisms sharing a
topology.
...
reproduced here. So this article ships no cover image and no figure out of the repository — the one
figure above is the Continual Harness paper's own methodology diagram, reproduced because Prime
Agent implements half of it, and the four interactives are original, built from reading the code and
measuring the repository, not redrawn from anything Prime Intellect published. The paper's
quantitative figures — the milestone-progress and cost-completion plots — do not render in arXiv's
HTML or in ar5iv, so they are not reproduced here either; they measure Gemini agents playing Pokémon
and say nothing about Prime Agent in any case.
