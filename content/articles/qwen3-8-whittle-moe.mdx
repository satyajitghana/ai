...
For scale: DeepSeek-V3 is 5.5% active, Mixtral 8×7B was 27.6%, and the [Switch Transformer](/articles/switch-transformer) that started this whole line of work routed each token to exactly one expert of thousands. At 66.3%, calling this a mixture of experts is a category claim the arithmetic does not support. It is a dense 27B that skips a third of its FFN.
...
Mean 4.50°, median 4.02°, p95 7.39°, worst single row 15.28°. The drift is monotone with depth — 3.41° averaged over layers 0–15, 6.28° over layers 48–63 — which is exactly where the findings log independently located the degeneration behaviour. And v1 to v2.1, across two more training rounds, the routers moved by a cosine of 0.9994.
...
A 16.8B depth-pruned model from the same author, on the same instrument, scores eight items higher than the 27B MoE — and unlike the 27-versus-28 wobble, that difference clears significance (Fisher exact, p = 0.036). The per-item run is in the repository as `research/q38_battery2_healed.json`. On the author's own evidence, the cheapest way to get a good small model out of Qwen3.8-27B is to cut layers off it, not to turn it into a mixture of experts.
...
**And the k-sweep refutes the author's own preferred story.** When the community suggested the looping was capacity starvation at top-16, the author served the same quantized weights three times with only `expert_used_count` overridden, verified the change was real from the throughput signature (18.8, 17.2, 15.9 tok/s at k = 16, 24, 32), and reported that k = 24 changes nothing row for row and k = 32 is worse. Publishing the result that kills your own hypothesis, with the confounds stated, is the thing.
...
**What I would watch.** Whether anyone runs the obvious experiment. `Qwen/Qwen3.8-27B` is Apache-2.0, `loop_test.py` is 200 lines of standard library, and `q38_battery2.py` is 100 more. Half an hour on a rented card produces the two numbers this entire campaign is missing, and they would settle whether a post-hoc MoE carved out of a dense model is worth anything at all relative to the model it came from. My prior, from the ladder in the author's own dev README, is that it is not — and I would rather be shown the measurement than keep the prior.

The sentence I keep coming back to is the one in `moe27_build.py`: *both are exact at uniform weights and the distillation pass refines the rest*. Everything downstream — the gibberish, the healing, the looping, five training runs and a year of a person's evenings — is the cost of "refines the rest" doing a lot of work in that sentence.
