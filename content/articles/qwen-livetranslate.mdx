...
competitor here, and one that published an actual paper — reports "3 seconds"
explicitly as the latency *of the cloned speech*. Qwen's own figure puts Seed at
2.8 LAAL. Both can be true; they are measuring different things. Anyone
comparing Qwen's 2.3 to Seed's 3.0 across the two announcements is comparing a
text clock to an audio clock.
...
That is all of it. There is no paper, no technical report, and no weights —
Qwen's Hugging Face organisation has never published a LiveTranslate checkpoint
in any generation, although it does publish open
[Qwen3-ASR](https://huggingface.co/Qwen/Qwen3-ASR-1.7B-hf) models. So the
mechanism has to be read off two sources: that paragraph, and the API.
...
$7.50 per million, 12.5 tokens per second of output audio at $30.00 per million.
