...
<Video
  src="/articles/kalman-filter/kalman-film"
  poster="/articles/kalman-filter/kalman-film-poster.jpg"
  narrated
  captions="/articles/kalman-filter/kalman-film.vtt"
  alt="A narrated, captioned hand-drawn film, 66 seconds, in eleven beats. A blue ellipse is drawn onto cream paper around a single dot, with construction lines and its two axes marked: a belief is a guess plus a shape. An arrow then pushes that ellipse to the right along a motion model and it grows as it travels, shedding specks. Three more predict steps leave a chain of pale ghosts behind a ballooning ellipse. A dotted sensor ray drops from the top of the frame, a red dot lands, and a tall narrow red ellipse blooms around it, overlapping the blue one. A small, densely hatched blue ellipse appears in the overlap; the two input ellipses fade and slide onto its centre, where the small one is visibly inside both of them. An ink blot opens onto a chalk-on-navy blueprint: two broad bell curves, pale blue and red, and between them a tall narrow white curve whose peak clears a dashed rule drawn above both. Back on paper, a bead slides along a rail between a blue ellipse at one end and a red dot at the other. With the bead at the red end the estimate jumps onto every scattered red measurement; with it at the blue end the estimate sails on in a straight line past them; settled in between, it follows a smooth path near the measurements without chasing them. A left-to-right sweep then shows a filter running: a faint true curve, red measurements, and a blue uncertainty band that widens between them and pinches at each one, sawtoothing across the frame. It closes on the hand-lettered words 'predict / then update'."
  caption="The loop as geometry, in 66 seconds. Press play — it is narrated and captioned. Every frame is drawn by JavaScript on a 2D canvas at 12fps and voiced with Kokoro-82M on CPU: a drawn explanation, not a recording. The scatter, the gain lever and the sizes of the ellipses are illustrative and measure nothing. Two things in it are exact rather than drawn. The small ellipse in the update shot is the real normalised product of the two on screen, so when all three are slid onto one centre it lands inside both — that containment is the covariance inequality, not an artistic choice. And the sawtooth band at the end is a real scalar filter run, its width the actual square root of P. The arithmetic you can check is below: the Gaussian-product figure, the matrix equations, and the runnable tracker with R = 49. (Drawn with the vendored hand-drawn-canvas-animation skill, MIT, Alexey Fateev.)"
/>
