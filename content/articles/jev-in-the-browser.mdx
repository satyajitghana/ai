...
description: "I built it: a per-option System One scorer on WebGPU/WASM through transformers.js, loaded on click, live in the page. It runs. Three things measuring it taught me. The isolation property that defines Jev costs 6.2× the tokens of the shared-sequence alternative at sixteen options, measured with one tokenizer on identical content — and both open System One models already gave that property up, which is why they are fast. The only one with a browser playground exports to ONNX with its logits fixed at [batch, 25], so the option count that was a loop bound in PyTorch becomes a tensor shape at the edge. And the ladder on a CPU is not small-and-fast versus big-and-slow: it is 437 ms and wrong versus 11.4 s and right, with nothing in between."
date: 2026-09-19
interest: 4
helpful: 5
featured: true
...
<Receipts src="/articles/jev-in-the-browser/data/quality-ladder.json" />
...
<Receipts src="/articles/jev-in-the-browser/data/redundancy.json" />
...
list and each gets a third of the mass.

Now put a policy on top, which is the entire reason this category exists — *act
autonomously above 0.7, escalate below*. That policy fires at five options and
abstains at seven, on identical evidence, because somebody added synonyms to a
...
share a sequence at least *can* learn to. That is the second time in this article
the same trade has shown up, and it is the deeper reason it matters.
