...
Cosmos ships in two sizes, both **sparsely activated**: **Nano — 16B total, 8B dense-active**, initialized
from **Qwen3-VL-8B**; and **Super — 64B total, 32B dense-active**. The Qwen initialization is telling — the
reasoner tower inherits a strong pretrained VLM, and the diffusion tower is grafted on and trained to read
it. The license is **OpenMDW-1.1**, an open model-weights license.

## The numbers, honestly

Cosmos leads a physical-AI reasoning leaderboard — **among open models**. That scope matters, and it is
easy to lose in a press release. On the benchmark where Cosmos Super posts its headline result, a
closed frontier model still sits above it:
