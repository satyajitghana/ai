...
Here is the sweep as Thinking Machines plot it, which is the measurement the interactive above is drawn
from:

<Figure
  src="/articles/inkling/fig1.png"
  alt="Three scatter panels — Terminal Bench 2.1, Humanity's Last Exam and IFBench — each plotting score against mean generated tokens on a linear axis. In every panel a blue line traces Inkling's effort sweep, rising steeply at low token counts and then flattening; GLM-5.2, Kimi K2.6, Nemotron 3 Ultra, Kimi K2.5 and GPT-OSS (high) are shown as single coloured dots at their default operating points. On Terminal Bench 2.1 Inkling's curve reaches Nemotron 3 Ultra's height at roughly a third of Nemotron's token count, while GLM-5.2 and Kimi K2.6 score higher than Inkling's plateau and do it at more tokens."
  caption="The effort sweep, 0.2 to 0.99, against mean generated tokens. The left panel is where the one-third-of-the-tokens claim comes from — Inkling's curve crosses Nemotron 3 Ultra's operating point well to its left. The post footnotes that the HLE panel reflects an earlier checkpoint running slightly below the final release. (Thinking Machines Lab, Inkling announcement post — the lab draws this chart in the page rather than publishing an image, so this is a capture of its own rendering.)"
/>
...
<Figure
  src="/articles/inkling/fig2.png"
  alt="A line chart of aggregate eval reward against RL rollouts on a logarithmic x-axis running from zero to more than thirty million. A grey open circle marks the SFT initialization at 0.264, a series of blue dots marks intermediate RL checkpoints, and a green dot marks the released checkpoint at 0.356 at roughly 26 million rollouts. All points sit close to a single straight line."
  caption="The log-linear claim as the lab plots it: reward on a held-out aggregate of reasoning evals — AIME, HLE, GPQA and others — climbing from 0.264 at SFT initialization to 0.356 at release, with the checkpoints hugging a straight-line fit. The checkpoint around 6M rollouts is the one that sits clearly under the fit. (Thinking Machines Lab, Inkling announcement post.)"
/>
...
<Figure
  src="/articles/inkling/fig3.png"
  alt="A ten-axis radar chart comparing Inkling against Nemotron 3 Ultra, GLM 5.2, GPT 5.6 Sol and Claude Fable 5 on a shared 0 to 100 scale. The axes are labelled with their category: HLE (no tools) and AIME 2026 for reasoning, SWE-bench Pro (Public) and Terminal Bench 2.1 for agentic coding, MCP Atlas for general agentic, SimpleQA Verified and AA Omniscience for factuality, AudioMC for audio, MMMU Pro for vision, and IFBench for chat. Inkling's solid blue polygon is the only one that extends along the AudioMC axis; on the two factuality axes and on SWE-bench Pro it sits inside Claude Fable 5 and GPT 5.6 Sol."
  caption="The lab's own summary chart, and a clear look at the comparison set the caveat below is about: five provider-selected models on ten provider-selected axes, all flattened onto one 0–100 scale. Inkling's unambiguous lead is the audio axis, where all four comparison models sit at the centre of the chart. (Thinking Machines Lab, Inkling announcement post.)"
/>
...
noted inline. The three static figures are the announcement post's own charts, reproduced as published —
the Hugging Face repository ships no figures of its own, and the model card page carries none either.
Related reading:
