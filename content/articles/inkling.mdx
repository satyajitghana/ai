...
import { NetworkDiagram } from "@/components/articles/inkling/network-diagram"
...
Put together, one token's journey through Inkling looks like this — three modalities tokenized into a
single **6144-dim** stream, an embedding norm at entry, **66 blocks** of hybrid attention plus a
mixture-of-experts FFN, and two output heads at the top:

<NetworkDiagram />

A few things the diagram makes concrete. The **MoE router** is deliberate in its details: a **sigmoid gate**
(not softmax) with a learned bias picks the top-6, the gate scores are **normalized *after* the top-k
selection** and multiplied by a fixed **route scale of 8**, and the **2 shared experts act as a "sink"** —
always-on capacity that absorbs whatever the routed experts don't specialize in. And not every layer is even
a mixture: an early **dense MLP** (a fat 24,576-wide FFN) sits among the expert layers, the common "keep the
first block dense" move. Two more stability touches surface only in the config — **attention log-scaling**
past a 128k-token floor, so scores don't blow up at extreme context, and **muP**-style logit scaling — the
unglamorous machinery of training a trillion-ish-parameter model without it diverging. Even the norms are a
theme: there's a dedicated RMSNorm not just on the text embeddings but on the **audio and vision** streams too.

The part the announcement buried entirely: Inkling ships **8 multi-token-prediction layers**. Beyond the main
LM head, the model is trained to predict several future tokens at once — the same
**[multi-token prediction](/articles/multi-token-prediction)** trick that gives a model a built-in draft for
**self-speculative decoding**, verifying several tokens per forward pass. For a model whose whole pitch is
cheap, controllable inference, an 8-deep MTP stack isn't a footnote — it's part of the efficiency story, and
it pairs naturally with the controllable-effort knob below.
