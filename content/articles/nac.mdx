...
<Figure
  src="/articles/nac/fig1.png"
  alt="A flow diagram. Three inputs on the left — 'Named source threads: latest episode from each' (a dashed arrow labelled 'context only'), 'Bounded action: one goal, one dispatch', and 'Target thread: retained history' — converge on a box labelled 'Fresh worker, clean context window'. Three outputs on the right: 'New episode, committed to target thread' (labelled 'final response'), 'Updated target thread, history persists', and 'Environment changes, remain'. A dashed arrow drops below the worker: 'Execution context discarded when the process exits'."
  caption="What a worker is given and what is left of it. Three inputs, three outputs, and the transcript that produced the episode is not one of them. Arcee publishes this diagram as an animation rather than an image file, so this is a capture of its final state — what a reader of the post ends up looking at (Arcee's nac write-up, arcee.ai/blog/nac)."
/>
...
<Figure
  src="/articles/nac/fig2.png"
  alt="A diagram of three threads. Thread A accumulates one episode A1; Thread B accumulates B1 then B1 and B2. Dashed arrows carry 'latest episode: A1' and 'latest episode: B2' into a fresh worker advancing Thread C, which starts with no episodes and ends holding only its own episode C1."
  caption="Thread weaving, drawn: the latest episodes of A and B reach the worker advancing C as context, and C's history ends up holding C1 alone. Captured from the post's animated figure at its final state, as above (Arcee's nac write-up, arcee.ai/blog/nac)."
/>
