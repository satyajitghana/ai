...
That figure is genuinely good, and it is good for reasons the stated standards
do not mention.

## The stated standards, sorted

The README and `references/design-theory.md` state the house rules. Here they
are with my classification, and with what the repo's own 25 scripts actually do.

| Rule | Legibility or convention | Complied with |
|---|---|---|
| Top and right spines off | Legibility — removes two lines that encode nothing | 19 of 25 scripts |
...
| `svg.fonttype='none'` for editable vector text | Legibility, for the next editor | Set, but 25 of 28 saves are PNG |
| `font.family = 'helvetica'` | Convention, and broken off the author's machine | 19 of 25 scripts |
...
deuteranopia**. For calibration: 5.2 is roughly the separation between two
*adjacent steps of the same red ramp* in `figure_CellSpliceNet` — 5.2 — which
are meant to read as shades of one colour. The two semantic opposites become
...
I also ran the skill's `DEFAULT_COLORS` through the `dataviz` skill's palette
validator, which is an independent implementation with published thresholds:
...
$ node scripts/validate_palette.js "#3775BA,#8BCF8B,#E9A6A1" --mode light
  ...
  [FAIL] CVD separation      worst adjacent #E9A6A1 ↔ #8BCF8B ΔE 2.3 (deutan)
...
`figure_ophthal_review/plot_trend.py` does it properly — the multimodal panel is
distinguished from the text-only panel by `hatch='///'` versus `hatch='\\\\\\'`
rather than by hue — and `figure_Brainteaser` hatches its four subtypes. That is
...
The one-line fix for the whole problem is to swap the reds for oranges or
purples, which keeps the semantic scheme, keeps blue-for-mine, and survives the
simulation. Nothing about the style has to change.
...
enforce is exactly what is lost. The 25 real scripts do not use these helpers
...
produce one: the palette is a dict, the rules are numbers, and there are 25
...
from advice into a gate. It would also have caught that 10 of 39 bar calls skip
the black edge the standard asks for.
...
`matplotlib 3.11.2` and ran all 25 scripts.

- **20 ran.** They wrote figures at the committed pixel dimensions.
- **4 failed** on LaTeX. They set `text.usetex = True` and need a TeX
  installation with `cm-super` and `dvipng`; `latex` alone is not enough.
- **1 failed** on `import seaborn`.
- There is no `requirements.txt`, `environment.yml` or `pyproject.toml`.

The 20 that ran did not reproduce the committed PNGs. Between 4.5% and 18% of
...
`plt.rcParams['font.family'] = 'helvetica'` appears in 19 of 25 scripts. It is a
single non-generic family name with no fallback list, so on any machine without
...
no `min` prop. That is not virtue; it is that nobody added the knob. figures4papers
has the knob and a rule encouraging its use. I would keep our version.
...
component, so a figure is correct in light and dark mode. `components/articles/DIAGRAM_GUIDE.md`
makes that a hard constraint. The two components in this article had to put their
swatches on an explicit `fill="#ffffff"` plate, because a palette pinned to
...
**They have semantic colour; we do not, and should.** Across 258 articles our
components each pick their own accent. figures4papers' reader learns one mapping
and carries it across eight papers. That is real and we do not have it. The
...
copy-paste and I cannot prove it. The point is that committing the data next to
the script is what made the question askable at all.
...
- **Swap the reds.** Orange or purple against green keeps the whole semantic
  scheme and survives dichromacy. Or keep the reds and hatch them — the code to
  do it is already in `figure_ophthal_review`.
...
<Falsifier claim="20 of 25 scripts run on a bare numpy + matplotlib install; the other 5 need undeclared dependencies.">
Container with Python 3.11, numpy 2.4.6, matplotlib 3.11.2, `latex` and `pdflatex` present, no `dvipng`, no `cm-super`, no seaborn. Four `text.usetex` scripts fail in `tight_layout` when LaTeX cannot typeset a tick label; `figure_ophthal_review/plot_composition.py` fails on `import seaborn`. A machine with a full TeX Live and seaborn installed runs 25 of 25, which is presumably the author's. The claim is about what a clean clone reproduces, and a committed `requirements.txt` plus a note about `cm-super` would settle it.
</Falsifier>
...
commit `3c181f85e82c6f24948fcaaf3be6696102b41d8d`, 76 tracked files, 25 Python
scripts. Repository CC BY-NC 4.0; the three figures reproduced above are the
