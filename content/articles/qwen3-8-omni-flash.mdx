...
The setup, from earlier in the same clip: a quadruped robot in an office, a phone face-up on the carpet, and a person who says — the transcript bubble carries a waveform icon, so it is spoken, not typed — "please go check, what is making the noise". The demo then names each stage on screen as it happens: *Prioritize Human Voice Commands*, *Understand Voice-Based Tasks*, *Determine sound direction and distance*, *Observe the Environment and Detect Obstacles*, *Reason in Real Time to Plan Navigation Routes*, *Generate Trajectories*, *Locate the Sound Source*.

What makes this worth more than the usual launch reel is that Qwen leaves two panels on screen at the same time and never connects them. On the left, the model's own reasoning, transcribed from the frame at 55.5 seconds:
...
Directly below it, in the same frame, a `Trajectory Output` table: eight waypoints, each an `x`, a `y` and a `yaw`, in the robot's own body frame. Qwen prints both panels and comments on neither. They are checkable against each other.

<SoundTrajectory />

Waypoint 8 sits 1.101 m forward and 0.492 m right of where the robot started. `hypot(1.101, 0.492) = 1.2059 m`. The model said 1.2. The bearing works out to 24.1 degrees right of straight ahead, which is "front-right" as claimed. Perception and planner agree to six millimetres.

That is a consistency check, not a measurement — worth being precise about which. Nothing in the frame tells you how far the phone actually was; it tells you the trajectory Qwen's stack emitted is the trajectory you would emit *if you believed the distance the model reported*. What it does rule out is the cheaper reading, the one worth suspecting in any vendor demo: that "a medium distance of 1.2 meters" is decorative narration generated beside a path computed by something else entirely. The two numbers are coupled. It is still one take, on one phone, chosen by the vendor.

The shape of the path is the other thing the table gives away. Five advances of exactly 0.25 m and three turns of exactly 15 degrees — the yaw column reads 0, −0.262, −0.524, −0.785 rad, which is 0°, −15°, −30°, −45° to three decimals. The robot walks 1.25 m of path to close a 1.206 m gap. That is a discrete action vocabulary — *step a quarter metre*, *turn fifteen degrees* — emitted as a plan, not continuous servoing toward a sound. Useful to know before reading "locates targets by sound" as something closer to a homing missile than to a planner that got told a bearing.
