...
**Update, 2026-09-26:** that changed the day this piece was published. stable-diffusion.cpp
commit `2dc7f54` ([#2035](https://github.com/leejet/stable-diffusion.cpp/pull/2035), 23 September)
adds a prefix KV cache for Qwen-Image-2.1 and turns it on by default: the first denoising call
saves the text and reference-image keys and values from every layer, and later calls compute
only the target-image tokens. Its docs put the cache at about 4 GiB in FP32 per condition for a
4,096-token prefix, and `--model-args qwen_image_2_1_prefix_cache=false` switches it off. The
arithmetic below describes builds before `2dc7f54`, which includes the one every CPU number on
this site was measured with. On a current build a longer rewrite no longer runs through all 32
blocks on every step; the image tokens still attend to its cached keys and values, so it still
costs something per step, but far less than the figure below (**Reasoned**). The runtime itself
is covered in [stable-diffusion.cpp](/articles/stable-diffusion-cpp).
