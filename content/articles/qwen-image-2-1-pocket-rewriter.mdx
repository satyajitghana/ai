...
## The run on four cores: not yet done

This section is a plan, not a result. I have not run either pocket model or the 9B,
and I have not generated a single image from a rewritten prompt. The machine that builds
this site has four cores. They are currently generating and timing the gallery for the
parent article, and anything else I ran would corrupt its per-step timings. When that
run finishes, this section will hold:

- **The rewriters on CPU.** The 0.8B through `llama.cpp` from its own Q8_0 GGUF and
  through `transformers` from the safetensors, then the 2B, then the 9B from a
  community Q4_K_M GGUF. Each gets the same gallery prompts. For each I will record
  tokens in, tokens out, thinking tokens where there are any, wall-clock and peak RSS.
  The 9B arm turns the teacher's cost from Reported into Measured.
- **The test that matters.** Four of the parent gallery's prompts, re-rendered from the
  rewrites with exactly the gallery's settings and seed, each set next to the raw-prompt
  image the gallery already has. I chose the four before running anything, and every
  pair gets published, including the ones where the rewrite made the picture worse:
  - the **transparent glass bottle**, because it depends on a trigger sentence the
    rewriters have never seen;
  - the **conference poster** with five quoted strings, the category the students saw
    eleven examples of;
  - the **bilingual noodle-shop sign**, Han and Latin script in one string;
  - the **three cups and two bowls**, where the counts and positions have to survive the
    rewrite.

  The gallery renders square, and the rewriters choose a ratio. I will hold the square
  and discard their choice, so the pairs test the prompt text and nothing else.
