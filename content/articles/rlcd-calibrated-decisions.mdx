...
<Video
  ...
  alt="A hand-drawn film, 28 seconds, in five beats. An orange cartoon figure labelled AN LLM holds a pen while lines of handwriting pour across the page. A blue figure labelled JEV appears instead beside three cards reading REFUND, REPLACE and ESCALATE; it points, the middle card lights yellow, and a card reading 0.70 rises above it. Jev then stands beside a ten-by-ten block of marks — seventy green ticks and thirty pink crosses — over two bars labelled CLAIMED and WAS RIGHT that come out the same length, and a green tick appears beside them. A pink figure holding a card reading 0.99 stands over the identical block of marks, but now its CLAIMED bar overshoots the WAS RIGHT bar and the shortfall between them is hatched and labelled THE GAP, with a pink cross beside it. The word RLCD appears large; 'Constrained Decoding' is struck through in pink and replaced by 'Reinforcement Learning for Calibrated Decisions'. It closes on 'it does not write / it decides, and it says how sure'."
  caption="What Jev is, and what RLCD is named after. Drawn frame by frame in JavaScript on a 2D canvas at 12fps, typeset in the site's own Hanken Grotesk. The hundred ticks and crosses are an illustration of what calibration means operationally — not a measurement of Jev, which has no published calibration number anywhere. The measured reliability numbers in this piece come later, and they are openjev's, not TypeSafe's. (Drawn with alesha-pro/tools' hand-drawn-canvas-animation skill.)"
/>
