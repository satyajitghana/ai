...
That is also how [BitNet b1.58](https://arxiv.org/abs/2402.17764), the paper this model descends from, draws the
change. One row of a ternary weight matrix against an input vector stops being a string of multiply-then-add and
becomes `x₀ − x₁ − x₂ + x₃` — the same arithmetic the toggle above counts:

<Figure
  src="/articles/ternary15m/fig1.png"
  alt="Two stacked panels. In the upper panel, performance is plotted against cost: BitNet b1.58, whose weight matrix W contains only the values 1, 0 and −1, sits on the same horizontal performance line as a transformer LLM whose W holds 16-bit floats, but far to the left, with an arrow labelled Pareto Improvement pointing from the float model toward the ternary one. In the lower panel the same weight matrix is multiplied by an input vector twice: the FP16 row yields 0.2961·x0 − 0.0495·x1 − 0.0924·x2 − 0.4765·x3, annotated Multiplication-then-Addition and routed to a GPU, while the ternary row yields x0 − x1 − x2 + x3, annotated Addition (no Multiplication) and routed to New Hardware."
  caption="The claim and the mechanism in one figure: ternary weights hold the performance line while moving left on cost, because a row of the matmul collapses from multiply-then-add into plain signed addition (BitNet b1.58, Figure 1)."
/>
...
"small floats." BitNet's own accounting puts a number on how much of a matmul's energy the multiply is:

<Figure
  src="/articles/ternary15m/fig2.png"
  alt="Two panels. On the left, a stacked bar chart of 7nm energy cost per arithmetic operation in picojoules: BitNet b1.58 is a thin sliver of INT8 addition near zero, while LLaMA's bar reaches 0.5 pJ, split into roughly 0.16 pJ of FP16 addition and roughly 0.34 pJ of FP16 multiplication, with a red double-headed arrow between the two bars labelled 71.4x. On the right, a log-scale line chart of end-to-end energy in joules against model size from 1.3B to 70B parameters: BitNet b1.58 runs below LLaMA at every size, and the annotated gap widens from 18.6x at 1.3B to 41.2x at 70B."
  caption="Where the multiplies sit in the energy budget: at a 7nm node the FP16 multiply is about two thirds of a matmul's arithmetic energy, and deleting it is worth 71.4× on arithmetic alone (left) and 18.6×–41.2× end to end as models grow (right) (BitNet b1.58, Figure 3)."
/>

Two things to keep straight about that figure. The 71.4× is arithmetic energy only — the multiply-and-add units, not
the memory traffic — which is why the end-to-end numbers on the right are an order of magnitude smaller than it. And
none of this was measured on Ternary15M: these are BitNet's projections for 1.3B-to-70B models at a 7nm node, and they
are the reason anyone builds ternary models, not evidence about the 15M one below.
...
is mine. The repo itself ships no figures, so the two reproduced above are BitNet b1.58's own (Figures 1 and 3) and
describe that paper's models, not this one — every number quoted about Ternary15M comes from its `RESULTS.md` and model
card.*
