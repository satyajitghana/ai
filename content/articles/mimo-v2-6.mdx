...
| audio encoder in the main weights | 268M | 261M |
...
blocks. Audio is two pieces: a 6-layer, 127M **patch encoder** in the main
weights that folds four frames into one token (25 Hz → 6.25 Hz), and a separate
...
it is a perfectly good thing to be at a tenth of the price.

Which is the other half of the story, and the card does not make it: at \$0.435
in and \$0.87 out per million tokens against Opus-class pricing, being nine rows
behind and one-fifth the cost is a straightforwardly attractive trade. That is
what the Pareto chart on the announcement is for, and it is a stronger argument
than parity.
...
evidence behind them. It also landed the same day as
