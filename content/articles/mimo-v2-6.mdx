...
On 2026-09-22 I could not find the framework or the environments on GitHub or as
a Hugging Face dataset — only the 9B checkpoint.

**Update, 2026-09-26:** they are out. The tasks are the Hugging Face dataset
[`XiaomiMiMo/MiMo-V2.6-RL-oss`](https://huggingface.co/datasets/XiaomiMiMo/MiMo-V2.6-RL-oss)
(7,780 rows: 6,780 across the four domains above plus 1,000 music tasks), the
task images are on Docker Hub, and the framework is the verl fork
[`XiaomiMiMo/verl`](https://github.com/XiaomiMiMo/verl). The validation splits,
the internal evaluation sets and the RL-trained 9B checkpoints are not in it.
I counted the files and read the graders in
[MiMo-V2.6's RL environments](/articles/mimo-rl-environments).
</Callout>
...
contribution than four points of index. The only thing wrong with it on
2026-09-22 was that I could not find it. It landed on 2026-09-26, and
[what is actually in it](/articles/mimo-rl-environments) is its own article.
...
Checked on 2026-09-22: no matching repository under github.com/XiaomiMiMo, no matching dataset under the XiaomiMiMo Hugging Face org, and the report's only footnote URL is the 9B checkpoint. If they land at a URL I did not look at, this is simply out of date — which is the most likely outcome and the best one. Update, 2026-09-26: it is out of date. Both are public now, as a Hugging Face dataset and a verl fork; the fork dates from 2026-09-21, and GitHub's search leaves forks out by default, which may be why I missed it. See [MiMo-V2.6's RL environments](/articles/mimo-rl-environments).
</Falsifier>
