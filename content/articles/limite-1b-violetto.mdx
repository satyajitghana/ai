...
models up to 753× its size. That is a real result and the BeyondAIME number in particular
...
Four repositories under `paradigma-inc`, all Apache-2.0 on the serving code and the weights:
`limite-1b-violetto` (the RL'd model), `limite-1b-base`, `limite-1b-base-soup` and
`limite-1b-value-model` — the training-time value model, which is an unusual and useful thing
to release. The base and Violetto configs are byte-identical apart from the weights, so the
