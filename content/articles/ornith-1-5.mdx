...
import { ImprovementLoop } from "@/components/articles/ornith-1-5/improvement-loop"
...
import { ScaleLadder } from "@/components/articles/ornith-1-5/scale-ladder"
...
<Figure
  src="/articles/ornith-1-5/fig1.png"
  alt="A flow diagram: the Ornith model, given an environment or codebase, emits a Task; the task branches into scaffold 1, 2 and 3; each scaffold branches into three solution rollouts; all nine rollouts feed a Reward node, and a GRPO 'RL update' arrow runs from the reward back to the model."
  caption="Ornith's own picture of the loop: one proposed task, several generated scaffolds, several rollouts under each, and a single reward that flows back through all three stages as one GRPO update (Ornith-1.5 release page, ornith.ai)."
/>
...
<Figure
  src="/articles/ornith-1-5/fig2.png"
  alt="A twelve-panel grouped bar chart titled 'LLM Performance Evaluation'. Each panel compares Ornith-1.5 (397B), Ornith-1.0 (397B), DeepSeek-V4-Flash-0731 (284B), GLM-5.2 (753B) and Claude Opus 4.8 on one benchmark: TB2.1, SWE-bench Verified, DeepSWE, Frontier-Bench v0.1, NL2Repo, SWE Atlas QnA, HLE, GPQA Diamond, MCP-Atlas, Toolathlon-Verified, WideSearch and BrowseComp."
  caption="The 397B flagship against its own previous generation and the frontier. The two orange bars in each panel are Ornith-1.5 and Ornith-1.0, which is where the uneven deltas are legible: DeepSWE 8 to 56 and Toolathlon 43.2 to 71.2 next to GPQA Diamond 88.1 to 92.8 (Ornith-1.5 release page, ornith.ai)."
/>
...
<Figure
  src="/articles/ornith-1-5/fig3.png"
  alt="A twelve-panel grouped bar chart comparing Ornith-1.5-35B-A3B, Ornith-1.0-35B-A3B, Qwen3.6-35B-A3B, Muse-Glimmer-30B (dense), Gemma-4-31B (dense) and, past a dashed divider, Qwen3.5-397B, across the same twelve benchmarks."
  caption="The middle rung, with its architectural peer in the same frame: Ornith-1.5-35B-A3B leads Qwen3.6-35B-A3B on every panel reported — 68.5 against 52.5 on TB2.1, 46.2 against 29.4 on NL2Repo, 22 against 0 on DeepSWE — and the dashed column on the right is Qwen3.5-397B, a model an order of magnitude larger (Ornith-1.5 release page, ornith.ai)."
/>
