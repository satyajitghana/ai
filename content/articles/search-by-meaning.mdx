---
title: "grep, but the pattern is a proposition"
...
<Figure
  ...
  caption="Two meanings in two languages against one mixed-language log, with -p printing each meaning's probability. Note line 5 — an INFO line about a retry — at 0.73 on 'network or remote connection failure'; it comes back later. (jev-semgrep, docs/color.svg.)"
/>
...
Here is the case as the README makes it. All six lines of `tests/contrast.txt` below are about a
refund. Only two are a customer asking for one.
...
is not a hunch — it is the finding of the benchmark built for exactly this:

<BenchBars
  title="NevIR: pairwise accuracy on document pairs that differ only by negation (Weller, Lawrie & Van Durme, arXiv 2305.07614)"
  unit="%"
  max={100}
  bars={[
    { label: "human", value: 100 },
    { label: "cross-encoder (MonoT5 3B)", value: 50.6, highlight: true },
    { label: "random baseline", value: 25 },
    { label: "late interaction (ColBERTv1)", value: 19.7 },
    { label: "bi-encoder (multi-qa-mpnet)", value: 11.1 },
    { label: "sparse (SPLADEv2)", value: 8.7 },
    { label: "TF-IDF", value: 2.0 },
  ]}
/>

Read the ordering, then read the baseline. On 2,556 document pairs that differ only by a
negation, the best cross-encoder gets 50.6% — half of human. Every bi-encoder, every sparse
retriever and TF-IDF score **below the 25% random baseline**, because they do not merely fail to
notice the negation, they systematically rank the same document above the other for a query and
for its opposite. The paper's own sentence: *"nearly all IR systems ignore the negation."*
...
step, no per-language index, no tuning, six languages found by a query in a seventh. It is also
...
`Failed to find Server Action`; and `serverActions.bodySizeLimit` defaults to 2 MB, which a
pasted log blows through.
...
Three details that matter. `revalidate: false` is right, and it is right for an unusual reason:
...
a 17.7% duplicate rate, but **86% of those duplicates are `---`, ` ``` `, `/>` and `</Callout>`**.
Strip trivial markup and the substantive repeat rate is **2.5%**.
