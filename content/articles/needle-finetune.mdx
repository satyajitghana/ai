...
Three other pieces of the Simple Attention Network are doing similar work:

- **The Hadamard MLP** replaces dense up-and-down projections with a fixed orthonormal Walsh transform — a matrix with no weights to read, applied in `n log n` — plus learned diagonals. Channel mixing dominates a small model's weight reads, and this makes it nearly free.
- **Multi-lane residual streams** give a 27-layer, 512-wide network the routing flexibility of a much wider one, at the cost of a few dot products per layer.
- **A 256-token sliding attention window** with the system prompt and tool declarations pinned as permanent sinks. Bounded KV cache regardless of session length, and the one thing a tool-calling model must never forget is structurally unable to be evicted.

That last one is why microcontrollers are reachable at all: the RAM ceiling is a deterministic 28 MB rather than a curve that grows with conversation length. ESP32-P4 with 32 MB of PSRAM, STM32H7, NXP i.MX RT. The engine compiles single-threaded for bare metal and ships as a static library for Cortex-M4, M7 and M55.
