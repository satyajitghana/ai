...
does not make that worse than GQA, which is the claim and it holds. But the same
week, [MiMo-V2.6](/articles/mimo-v2-6) shipped 60 of its 70 layers as
sliding-window attention with a 128-token window, so 6/7 of its layers stop
growing past 128 tokens.

Two releases, one day apart, making opposite bets about where attention should be
cheap: IFM buys representational capacity in the value path and pays full
freight on the cache; Xiaomi buys cache and pays with locality. Neither is wrong.
...
and it is a genuinely strong result — stronger, I think, than beating another
...
release that made the opposite bet on the same day.*
