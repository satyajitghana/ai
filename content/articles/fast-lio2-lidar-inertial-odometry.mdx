...
I tried to fix it the obvious ways — I added the Avia LiDAR→IMU extrinsic from the repo's
`avia.yaml` (`T = [0.04165, 0.02326, -0.0284]`, `R = I`), swapped in the real IMU
covariances (`acc_cov = gyr_cov = 0.1`, which my synthetic `Q` had 100× too small), and
switched to a proper gravity-aligned initialization. It still collapses. That stubbornness
is the most useful thing in this whole article, so here's exactly why it happens — and what
the real fix is:

- **A stationary fixed-point.** The map is seeded near the origin, and the over-confident
  point-to-plane update ($R = 10^{-3}$) snaps the pose back to match it — which also drives
  the *velocity* estimate to zero. With $v \approx 0$, the next IMU prediction barely moves
  the pose, the next scan again matches the origin map, and the filter settles into a
  self-consistent "I'm not moving" solution while the sensor swings through the room. It's a
  data-association bootstrap failure, not a bad equation.
- **Degenerate associations.** The Avia has a narrow ~70° FOV and a sparse, non-repetitive
  scan pattern, so a plain 5-NN plane fit on a cKDTree often finds planes that don't pin all
  six pose DOF — exactly when covariance can collapse in the unconstrained directions. Real
  FAST-LIO leans on the ikd-Tree, good initialization, and **gravity estimated in the state**
  (on $S^2$, which I fixed at init) to keep this from happening.
