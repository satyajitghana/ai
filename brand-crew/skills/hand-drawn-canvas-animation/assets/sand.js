...
function _sandTextures(w, h) { const key = w + 'x' + h; if (_sandTex.has(key)) return _sandTex.get(key); const n = w * h, gr = new Float32Array(n); let s = 99; const rnd = () => { s = (Math.imul(s, 1664525) + 1013904223) >>> 0; return s / 4294967296; };
  const w2 = Math.ceil(w / 2), w4 = Math.ceil(w / 4), n2 = new Float32Array(w2 * Math.ceil(h / 2)), n4 = new Float32Array(w4 * Math.ceil(h / 4)); for (let i = 0; i < n2.length; i++) n2[i] = rnd(); for (let i = 0; i < n4.length; i++) n4[i] = rnd();
  for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) gr[y * w + x] = .46 * rnd() + .34 * n2[(y >> 1) * w2 + (x >> 1)] + .2 * n4[(y >> 2) * w4 + (x >> 2)];
  const t = { grain: gr, img: new ImageData(w, h) }; _sandTex.set(key, t); return t; }
...
function sandImage(w = OUT_W, h = OUT_H, live = null) { const T = _sandTextures(w, h), N = SAND.N, hf = SAND.h, gr = T.grain, out = T.img.data, V = sandViewRect(w, h), kc = (N - 1) / SAND.world;
  ...
  for (let y = 0; y < h; y++) { const wy = wy0 + (y + .5) * stepy, fy = clamp(wy * kc, 0, N - 1.001), j = fy | 0, ty = fy - j, row = j * N, dv = (wy - ly) * inv;
    for (let x = 0; x < w; x++) { const wx = wx0 + (x + .5) * stepx, fx = clamp(wx * kc, 0, N - 1.001), i = fx | 0, tx = fx - i, p0 = row + i;
      ...
      const cov = 1 - Math.exp(-hh * 1.25); let a = (cov - gr[p]) * 6 + .5; a = a < 0 ? 0 : a > 1 ? 1 : a; const dep = hh > 2.6 ? 1 : hh / 2.6, dim = 1 - .3 * cov, q = p * 4;
...
function _handState(t) { const gs = SAND.gestures.filter(g => g.hand !== null); let act = null; for (const g of gs) if (t >= g.t0 && t <= g.t1) { act = g; break; }
  ...
  return null; }
function sandHand(c, t) { const st = _handState(t); if (!st || st.a <= .01) return; const q = 9; if (!_handCv) { _handCv = document.createElement('canvas'); _handCv.width = Math.ceil(W / q); _handCv.height = Math.ceil(H / q); } const g = _handCv.getContext('2d'); g.setTransform(1 / q, 0, 0, 1 / q, 0, 0); g.clearRect(0, 0, W, H);
