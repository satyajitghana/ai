...
// ============================================================
const CLIPS = {};
function registerClip(name, meta) { meta.name = name; CLIPS[name] = meta; }
...
function roto(c, name, k, o = {}) { const { x = CX, y = CY, h = 300, flip = false, rot = 0, fill = PAL.light, wash: washCol = null, ink = PAL.ink, weight = 1, seed = 1, p = 1, washOff = 7, washAl = .55, al = 1, amp = .5, maxLines = 9999 } = o;
  ...
  const n = Math.min(fr.lines.length, maxLines), upto = p * n; c.save(); c.globalAlpha = al;
  for (let i = 0; i < n && i < upto; i++) { const l = fr.lines[i]; brush(c, l.p.map(T), { w: Math.max(1.1, l.w * s * weight), color: ink, amp, seed: seed + i * 7, taper: .45, p: clamp(upto - i, 0, 1), smooth: l.p.length > 2 }); }
  c.restore(); }
...
function rotoSprite(name, k, h, o = {}) { const key = [name, k, h, o.wash || '', o.fill === null ? 'n' : o.fill || '', o.ink || '', o.weight || 1, S].join('|'); if (_sprites.has(key)) return _sprites.get(key);
