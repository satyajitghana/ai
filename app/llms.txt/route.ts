...
export function GET() {
  ...
  const lines = [
    ...
    "",
    "## Developer resources — API docs, OpenAPI spec, MCP server, auth (how to read this site)",
    "",
    ...
    `- API versioning & deprecation policy: ${absoluteUrl("/developers/versioning")} · machine-readable: ${absoluteUrl("/developers/versioning.json")}`,
    "- Every /api/* response carries an `API-Version` header; a deprecated version additionally carries RFC 9745 `Deprecation` and RFC 8594 `Sunset`",
    `- Homepage as markdown: ${absoluteUrl("/")} with \`Accept: text/markdown\``,
    "- Unknown paths return a real 404 whose body is a markdown recovery map (sitemap, llms.txt, section indexes) for any caller that did not ask for HTML",
